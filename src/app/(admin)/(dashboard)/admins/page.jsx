...
export default function AdminsPage() {
  ...
  const fetchAdmins = () => {
    setLoading(true);
    axios.get(`/api/admin/admins?search=${debouncedSearch}`)
  ...
  const displayedAdmins = admins.filter(admin => {
    if (statusFilter === 'active') return admin.is_active;
    if (statusFilter === 'suspended') return !admin.is_active;
    return true;
  });
    ...
    <div className="space-y-6">
                ...
                {loading ? (
                  ...
                  displayedAdmins.map((admin) => (
                    <TableRow key={admin.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <TableCell>
                        <Badge variant="outline" className="text-[10px] uppercase tracking-wider text-slate-500 border-slate-200 bg-white">
                          {admin.role}
                        </Badge>
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
        <DialogContent className="sm:max-w-[425px] bg-white">
          <DialogHeader>
            <DialogTitle className="text-slate-900">Add New Administrator</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleAddAdmin} className="space-y-4 pt-4">
            ...
            <DialogFooter className="pt-4">
              <Button type="button" variant="outline" className="bg-white text-slate-700 border-slate-300 hover:bg-slate-50" onClick={() => setIsAddModalOpen(false)}>
                Cancel
              </Button>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[425px] bg-white">
          <DialogHeader>
            <DialogTitle className="text-slate-900">Edit Administrator Details</DialogTitle>
          </DialogHeader>
          {editData && (
            <form onSubmit={handleEditAdmin} className="space-y-4 pt-4">
              ...
              <DialogFooter className="pt-4">
                <Button type="button" variant="outline" className="bg-white text-slate-700 border-slate-300 hover:bg-slate-50" onClick={() => setIsEditModalOpen(false)}>
                  Cancel
                </Button>
                <Button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white" disabled={submitting}>
